...
export default function FichaDeTreinoScreen({ user, onLogout, onOpenAdmin }) {
  ...
  const currentWorkout =
    selectedWorkoutKey !== null
      ? availableWorkouts[selectedWorkoutKey]
      : availableWorkouts[today];
    ...
    const fetchCustomWorkouts = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const allExercisesFlat = Object.values(workoutData).flatMap(
            (day) => day.exercises || [],
          );
          const expandedWorkouts = { ...workoutData };

          data.forEach((cw) => {
            const assignedExercises = cw.exercises
              .map((exId) => allExercisesFlat.find((e) => e.id === exId))
              .filter(Boolean);
    ...
    if (voiceTimerActive) {
      ...
      plankIntervalRef.current = setInterval(() => {
        setPlankTime((prev) => {
          ...
          if (next % 60 === 0 && next > 60)
            speak(`${next / 60} minutos. Performance extrema.`);
          return next;
        });
      }, 1000);
    }
  ...
  const NavButton = ({ id, icon: Icon, label }) => (
    ...
    >
      ...
      <span className="text-[10px] uppercase font-black mt-1 tracking-tighter">
        {label}
      </span>
      {activeTab === id && (
        <motion.div
  ...
  const GlassCard = ({ children, className = "", gradient = false }) => (
    <div
      className={`relative overflow-hidden bg-neutral-900/50 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-[0_0_20px_rgba(37,99,235,0.05)] hover:border-yellow-500/30 transition-all duration-500 ${className}`}
    >
      {gradient && (
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none opacity-20 overflow-hidden z-0">
        ...
        <div
          className="absolute top-1/2 -right-24 w-64 h-64 bg-indigo-600 rounded-full blur-[100px] animate-pulse"
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="bg-black/10 p-1 rounded-md hover:bg-black/20 transition-colors"
              >
        ...
        {isTraining && (
          <div className="max-w-xl mx-auto mt-3">
            ...
            >
              ...
              <span className="text-xs font-black font-mono text-red-500 tracking-tighter">
                SESSÃO ATIVA: {formatTime(sessionTime)}
              </span>
            </motion.div>
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "painel" && (
            <TabPainel
              ...
              water={water}
              waterGoal={waterGoal}
              isHydrationAlert={isHydrationAlert}
              handleWaterDrink={handleWaterDrink}
              protein={protein}
          ...
          {activeTab === "perfil" && (
            <TabPerfil
              user={user}
              today={today}
              voiceTimerActive={voiceTimerActive}
              toggleVoiceTimer={toggleVoiceTimer}
              formatPlankTime={formatPlankTime}
              plankTime={plankTime}
              onLogout={onLogout}
            />
          )}
      ...
      <AnimatePresence>
        {fabOpen && (
          <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
            {[
              { icon: Zap, label: 'Iniciar Sessão', color: 'from-yellow-400 to-amber-500', action: () => { startSession(); setFabOpen(false); } },
              { icon: Droplets, label: 'Água +250ml', color: 'from-cyan-400 to-blue-500', action: () => { handleWaterDrink(250); setFabOpen(false); } },
              { icon: Beef, label: 'Proteína +30g', color: 'from-red-400 to-rose-500', action: () => { setProtein(prev => prev + 30); setFabOpen(false); } },
              { icon: Coffee, label: 'Coach IA', color: 'from-purple-400 to-violet-500', action: () => { setActiveTab('coach'); setFabOpen(false); } },
              { icon: Camera, label: 'Foto Evolução', color: 'from-green-400 to-emerald-500', action: () => { setActiveTab('progress'); setFabOpen(false); } },
              { icon: Scale, label: 'Registrar Peso', color: 'from-orange-400 to-amber-600', action: () => { setActiveTab('perfil'); setFabOpen(false); } },
            ].map((item, index) => {
              const totalItems = 6;
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className={`h-12 w-12 rounded-full bg-gradient-to-br ${item.color} flex items-center justify-center shadow-lg shadow-black/40 border-2 border-white/20 group-hover:scale-110 group-active:scale-90 transition-all duration-200`}>
                      <item.icon size={20} className="text-white drop-shadow-md" strokeWidth={2.5} />
                    </div>
